...
export default class App extends Component {
	...
	renderConfirmationString() {
		...
		return this.state.confirmationTextVisible ? (
			...
			>
				{
					<span>
						Scheduling a<span style={spanStyle}> 10 Minute </span>
						appointment{" "}
			...
			return (
				<section>
					<TextField
						...
						onChange={(evt, val) =>
							this.setState({ admin_appointment_time: val })
						}
					/>
					<RaisedButton
						style={{ display: "block" }}
						...
						fullWidth={false}
						onClick={() =>
							this.handleSetAppointmentSlot(this.state.admin_appointment_time)
						}
						disabled={false}
		...
		if (!this.state.loading) {
			...
			return slots.map((slot) => {
				...
				const meridiemDisabled = this.state.appointmentMeridiem
					...
					: t1.format("a") === "pm";
				const time =
					(cur_hour > 12 ? cur_hour - 12 : cur_hour) +
					":" +
					(number_start == 0 ? "00" : number_start) +
					(cur_hour > 12 ? " PM" : " AM");
				return (
					<RadioButton
	...
	render() {
		...
		const contactFormFilled =
			(data.phone && data.validPhone) || this.state.phone_number;
		const modalActions = [
			<FlatButton
				label="Cancel"
				primary={false}
